...
import { Box, Button, Modal, MenuItem,  Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField } from "@mui/material";
...
const Order = () => {
  ...
  const [GrandTotalCash, setGrandTotalCash] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (salesRoute && selectedWarehouse) {
        try {
          const response = await fetch(`${process.env.REACT_APP_API_URL}/commerce/get-latest-price/${salesRoute}/${selectedWarehouse}`);
          if (!response.ok) {
...
const handleQpChange = (event) => {
  ...
  useEffect(() => {
    if (selectedRow && selectedRow.customers_id) {
      // Fetching ledger balance from API based on selected row's ID
      fetch(process.env.REACT_APP_API_URL + `/commerce/ledger-deposit-view/${selectedRow.customers_id}`)
        .then((response) => response.json())
  ...
  useEffect(() => {
    const totalp = parseInt(Qp || 0) + parseInt(Hp || 0) + parseInt(ONEp || 0) + parseInt(TWOp || 0);
    setTotalp(totalp);
  }, [Qp, Hp, ONEp, TWOp]);
  ...
  useEffect(() => {
    ...
    const grand_totalCash = parseInt(Q_Total || 0) + parseInt(H_Total || 0) + parseInt(ONE_Total || 0) + parseInt(TWO_Total || 0) + parseInt(TransportationFee || 0);
    setGrandTotalCash(grand_totalCash || '');
  }, [Qp, Hp, ONEp, TWOp, Q_Total, H_Total, ONE_Total, TWO_Total, TransportationFee]);
  ...
  const handleApprove = async (id) => {
    ...
    formData.append('Inventory', selectedWarehouse);
...
<TableCell>WareHouse </TableCell>
    ...
    <option value="AdissAbaba">Adiss Ababa</option>
  ...
  <TableCell>0.35ml Total</TableCell>
  <TableCell>
    <input
...
<TableCell>Total Qty</TableCell>
  ...
  <TableCell> Grand Total Value</TableCell>
  <TableCell>
    <input
      type="number"
