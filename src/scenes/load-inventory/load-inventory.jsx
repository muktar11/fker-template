...
const LoadInventoryRequest = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+"/commerce/fetch-rawmtaerial/");
  ...
  const handleApprove = async (id) => {
    ...
    formData.append('Plate', plate_no)
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/load-rawmtaerial/${id}/`, {
        method: "PUT",
        body: formData,
      });
      const data = await response.json();
      
      toast.success("Logistics Has Been Approved Successful ");
      handleUpdateClick()
      handleCloseModal()
  ...
  return (
    <Box m="20px">
      <Header title="Logstics Inventory" subtitle="List of Orders that requires processing" />
      <ToastContainer />
      <Box
        m=" 0 0 0"
        height="100vh"
...
export default LoadInventoryRequest;
