...
const PromotionWareHouseStatus = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+"/commerce/status-rawmtaerial/Promotion/");
  ...
  const handleUpdateClick = async () => {
    try {
      const response = await (process.env.REACT_APP_API_URL+"/commerce/status-rawmtaerial/Promotion/");
      const data = await response.json();
      setCustomerData(data);
...
;
  ...
  return (
    <Box m="20px">
      <Header title=" Request Status Promotion" subtitle="List of Orders that requires processing" />
      <ToastContainer />
      <Box
        m=" 0 0 0"
        height="100vh"
...
export default PromotionWareHouseStatus;
