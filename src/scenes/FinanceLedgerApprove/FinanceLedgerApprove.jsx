...
const FinanceLedgerApprove = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      clearInterval(interval); // Clear the interval on component unmount to avoid memory leaks
    };
  }, []); 

  useEffect(() => {
    if (customerData.length > 0) {
      // Check if customerData is not empty and send a notification
     toast.warn('new ledger in the set!!!')
    }
  }, [customerData]);
  ...
  const columns = [
    ...
    { field: "Deposit_Amount", headerName: "Deposit_Amount", flex: 1 },
    { field: "previous_balance", headerName: "Previous Balance", flex: 1 },
    { field: "Balance", headerName: "Curret Balance", flex: 1 }
      ...
      <TableRow>
       ...
       <TableRow>
        <TableCell component="th" scope="row">
          ...
          <TableCell >{selectedRow.previous_balance}</TableCell>
      </TableRow>
