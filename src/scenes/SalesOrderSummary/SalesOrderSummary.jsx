...
const SalesOrderSummary = () => {
  ...
  const fetchDataWithFilters = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/commerce/filtered-agent-sales-orders/?${queryParams.toString()}`);
      const data = await response.json();
  ...
  const columns = [
    ...
    { field: "plate_no", headerName: "Plate", flex: 0.7 },
    { field: "sales_Route", headerName: "Route", flex: 0.7 },
  ...
  return (
    <Box m="20px">
      <ToastContainer />
      <Header title="Agent Sales Order Summary" subtitle="List of orders" />
      <Box mb={2}>
        <TextField
...
<div className="pack" style={{ display:"flex", flexDirection:"row", justifyContent:"space-between", alignItems:"center"}}>
    <div className="billing" style={{maxHeight:"160px", display:"flex", flexDirection:"column", marginBottom:"10px"}}>
        ...
        {selectedRow.is_mobile ? (
              <h8>Supervisor: {selectedRow.supervisor}</h8>
        ) : null}  
        ...
        <h8>Plate Number: {selectedRow.plate_no}</h8>
      ...
      <div className="billing" style={{maxHeight:"160px", display:"flex", flexDirection:"column", marginBottom:"10px"}}>
        ...
        <h8>CSIV No:  {selectedRow.CSI_CRSI_Number} </h8>
        <h8>ISIV NO/Do No:  {selectedRow.inventory_ddo} </h8>
...
<div className="body-title" style={{ textAlign: "center", marginTop:"50px"  }}>
    ...
    <div class="table-row" style={{ display:"table-row"}}>
      ...
      <div class="table-cell" style={{ display: "table-cell", padding: "8px", border:"1px solid black"}}>{selectedRow.sdmcreated_first_name} {selectedRow.sdmcreated_last_name}</div>
