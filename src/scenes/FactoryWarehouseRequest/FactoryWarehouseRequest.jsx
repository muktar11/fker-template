...
const  FactoryWareHouseRequest = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+"/commerce/access-rawmtaerial/AdissAbaba/");
  ...
  const columns = [
    ...
    { field: "warehoust", headerName: "WareHouse", flex: 1 },
    { field: "0.35ml", headerName: "Q", flex: 1 },
    { field: "0.6ml", headerName: "H", flex: 1 },
    { field: "1L", headerName: "ONE", flex: 1 },
    { field: "2L", headerName: "TWO", flex: 1 },
  ...
  const handleApprove = async (id) => {
    ...
    formData.append('issue_store_file', payment)
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/approve-rawmtaerial/${id}/`, {
        method: "PUT",
        body: formData,
      });
      const data = await response.json();
  ...
  const handleReject = async (id) => {
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/reject-rawmtaerial/${id}/`, {
        method: "POST",
        headers: {
  ...
  return (
    <Box m="20px">
      <Header title="Incoming Orders" subtitle="List of Orders that requires processing" />
      <ToastContainer />
      <Box
        m=" 0 0 0"
        height="100vh"
...
export default FactoryWareHouseRequest;
